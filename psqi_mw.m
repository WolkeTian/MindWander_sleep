...
beh_table = contents{2:end, :};

beh_subid = str2double(beh_table(:,1)); % 提取被试编号
psqi = str2double(beh_table(:,2));
mwscores = str2double(beh_table(:,14)); % mw量表总分
%% load fnc
fncdir = 'H:\fMRI_第一批1087人\Results\Power264ROIs\ConnMatrix.mat';
load(fncdir);
...
%  negtive links
%  {' Frontal_Inf_Tri_R (aal)';' Postcentral_R (aal)';' Putamen_R (aal)';' undefined';' Parietal_Inf_L (aal)'}
